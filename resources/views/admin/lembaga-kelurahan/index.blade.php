...
@section('content')
    ...
    <main class="bg-white dark:bg-gray-800 p-5 m-3 border-gray-200 dark:border-gray-700 rounded-lg shadow-sm">
        ...
        <div class="w-full mb-3">
            ...
            <div class="flex gap-3">
                ...
                <div
                    class="flex divide-x divide-gray-100 :divide-gray-700">
                    <form class="flex items-center max-w-sm mx-auto" action="{{ route('admin.lembaga-kelurahan.index') }}"
                          method="GET">
                        <label for="search" class="sr-only">Search</label>
                        <div class="relative w-full">
                            <div class="relative mt-1 lg:w-36 xl:w-64">
                                <input type="search" name="query" id="search"
                                       class="bg-blue-50 border border-gray-300 text-black-900 sm: rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-blue-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                       placeholder="Cari @yield('title')" value="{{ request()->get('query') }}">
                            </div>
                        </div>
                        <button type="submit"
                ...
                <div class="inline-block min-w-full align-middle">
                    <div class="overflow-hidden shadow">
                        <table id="table" class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                            ...
                            <tr>
                                <th scope="col"
                                    class="p-4 font-semibold text-left whitespace-nowrap text-black-500 uppercase dark:text-black-400 w-80">
                                    Nama
                                </th>
                                <th scope="col"
                                    class="p-4 font-semibold text-left whitespace-nowrap text-black-500 uppercase dark:text-black-400 w-80">
                            ...
                            @forelse($lembagaKelurahan as $lembaga)
                                ...
                                <tr class="hover:bg-blue-100 dark:hover:bg-blue-700">
                                    <td class="flex items-center p-4 mr-12 space-x-3 whitespace-nowrap">
                                        @if($lembaga->logo)
                                            <img class="w-10 h-10 rounded-full"
                                                 src="{{ asset('storage/'.$lembaga->logo) }}" alt="Logo">
                                        @endif
                                    ...
                                    <td class="p-4 space-x-2 whitespace-nowrap text-end">
                                        ...
                                        <button type="button" data-modal-target="edit-modal-{{ $lembaga->id }}"
                                                data-modal-toggle="edit-modal-{{ $lembaga->id }}"
                                        ...
                                        <button type="button" data-modal-target="delete-modal-{{ $lembaga->id }}"
                                                data-modal-toggle="delete-modal-{{ $lembaga->id }}"
                                        ...
                                        @include('admin.lembaga-kelurahan.edit', ['lembaga' => $lembaga])
                                        @include('admin.lembaga-kelurahan.delete', ['lembaga' => $lembaga])
                                ...
                                <tr>
                                    <td colspan="4" class="text-center py-3">
                                        @if(request()->get('query'))
                                            Pencarian @yield('title') <b>{{ request()->get('query') }}</b> tidak
                                            ditemukan
                                            <a href="{{ route('admin.lembaga-kelurahan.index') }}"
                                               class="underline">Kembali</a>
                                        @else
    ...
    @include('admin.lembaga-kelurahan.create')
